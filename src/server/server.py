...
async def api_create_task(req: Task):
    try:
        ...
        return {"message": "任务创建成功", "task": new_task_with_id}
    except Exception as e:
...
async def api_update_task(req: Task):
    try:
        ...
        return {'message': '任务更新成功', 'task': new_task}
    except Exception as e:
...
async def get_task_results(task_id: int, page: int = 1, limit: int = 20, recommended_only: bool = False, sort_by: str = 'crawl_time'):
    try:
        task = await get_task(task_id)
        if not task:
            raise HTTPException(status_code=404, detail=f"任务 {task_id} 未找到")
        return await get_task_result(task['keyword'], page, limit, recommended_only, sort_by)
    except Exception as e:
