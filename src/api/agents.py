...
@router.post("/templates/{template_id}/apply", dependencies=[Depends(verify_token)])
async def api_apply_agent_template(template_id: str):
    """应用Agent模板"""
    try:
        # 获取模板
        templates = AgentPresetTemplate.get_preset_templates()
        ...
        if not template:
            raise HTTPException(status_code=404, detail=f"模板 '{template_id}' 未找到")
        ...
        # 检查是否已存在
        existing_agent = await get_agent_config(template_id)
        if existing_agent:
            # 更新现有配置
            result = await update_agent_config(template_id, agent_config)
            message = "更新成功"
        else:
            # 创建新配置
            result = await add_agent_config(agent_config)
